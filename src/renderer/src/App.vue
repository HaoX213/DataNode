...
const COPILOT_WELCOME_GLOBAL: ChatMessage = {
  role: 'assistant',
  content:
    '你好，我是「全局 AI」助手。对话不与项目默认绑定；请在分支列表顶部用**文件夹 / 书本图标**为当前分支关联项目数据与书柜笔记。还可将对话「总结并存入知识库」以积累 LLM Wiki。左侧可管理**全局对话分支**。'
}
...
const globalBranchLinkedProjectIds = ref<number[]>([])
const globalBranchLinkedNoteIds = ref<number[]>([])
const bookshelfNotesPickerList = ref<Array<{ id: number; title: string }>>([])
const wikiExtractLoading = ref(false)
...
let globalBranchContextTimer: ReturnType<typeof setTimeout> | null = null
...
const copilotContextLabel = computed(() => {
  if (copilotMode.value === 'global') {
    const np = globalBranchLinkedProjectIds.value.length
    const nn = globalBranchLinkedNoteIds.value.length
    const parts: string[] = []
    if (np) parts.push(`本分支已关联 ${np} 个项目的表格摘要`)
    if (nn) parts.push(`已关联 ${nn} 条书柜笔记正文`)
    if (parts.length === 0) {
      return '全局 AI：本分支未关联项目/笔记；将使用对话历史，并在提问时检索 LLM Wiki。'
    }
    return `全局 AI：${parts.join('；')}。`
  }
...
const copilotModeBadge = computed(() => {
  if (copilotMode.value === 'global') {
    const ids = globalBranchLinkedProjectIds.value
    const nNotes = globalBranchLinkedNoteIds.value.length
    if (!ids.length && !nNotes) return '全局 AI'
    const names = ids
      .map((id) => projects.value.find((p) => p.id === id)?.name)
      .filter((n): n is string => Boolean(n))
      .slice(0, 3)
    const tail = ids.length > 3 ? `等${ids.length}项` : ''
    const noteHint = nNotes ? ` · 笔记×${nNotes}` : ''
    return names.length ? `全局 AI（${names.join('、')}${tail}）${noteHint}` : `全局 AI${noteHint}`
  }
  return `项目 AI · ${currentProject.value?.name ?? '—'}`
})

async function loadGlobalBranchContextLinks(): Promise<void> {
  if (copilotMode.value !== 'global') {
    globalBranchLinkedProjectIds.value = []
    globalBranchLinkedNoteIds.value = []
    return
  }
  ...
  const r = await window.api.getGlobalAiTopicContext(tid)
  if (r.success && r.data) {
    globalBranchLinkedProjectIds.value = r.data.projectIds ?? []
    globalBranchLinkedNoteIds.value = r.data.noteIds ?? []
  } else {
    globalBranchLinkedProjectIds.value = []
    globalBranchLinkedNoteIds.value = []
  }
...
function schedulePersistGlobalBranchContext(): void {
  const tid = activeGlobalAiTopicId.value
  if (tid == null) return
  if (globalBranchContextTimer) clearTimeout(globalBranchContextTimer)
  globalBranchContextTimer = setTimeout(() => {
    globalBranchContextTimer = null
    void window.api.setGlobalAiTopicContext(tid, [...globalBranchLinkedProjectIds.value], [...globalBranchLinkedNoteIds.value])
  }, 420)
}

async function loadBookshelfNotesForPicker(): Promise<void> {
  const r = await window.api.listBookshelfNotesForGlobalAi()
  bookshelfNotesPickerList.value = r.success && Array.isArray(r.data) ? r.data : []
}

async function summarizeBranchToWikiAction(): Promise<void> {
  if (copilotMode.value !== 'global') return
  const tid = activeGlobalAiTopicId.value
  if (tid == null) {
    ElMessage.warning('请先选择对话分支')
    return
  }
  wikiExtractLoading.value = true
  try {
    const r = await window.api.summarizeGlobalBranchToWiki(tid)
    if (!r.success) {
      ElMessage.error(r.message || '总结失败')
      return
    }
    ElMessage.success(r.message || `已写入 Wiki：${r.data?.title ?? ''}`)
  } finally {
    wikiExtractLoading.value = false
  }
}
...
async function selectAiTopic(topicId: number): Promise<void> {
  if (copilotMode.value === 'global') {
    ...
    await loadGlobalBranchContextLinks()
    return
  }
...
const sendChatMessage = async (): Promise<void> => {
  ...
  try {
    const result = await window.api.chatWithAi({
      ...
      global_ai_topic_id: copilotMode.value === 'global' ? topicId : undefined,
        ...
        <div class="copilot-sheet-header">
          <div class="copilot-header-text">
            ...
            <div class="copilot-subtitle">
              项目 AI 按项目隔离分支；全局 AI 独立存储。全局模式下在左侧分支栏顶部用图标关联项目与书柜笔记，并可把对话总结存入 LLM Wiki。
            </div>
          </div>
          <div class="copilot-sheet-actions">
            ...
            <el-button
              v-if="copilotMode === 'global'"
              size="small"
              type="success"
              plain
              :loading="wikiExtractLoading"
              @click="summarizeBranchToWikiAction"
            >
              总结并存入知识库
            </el-button>
            <el-button
